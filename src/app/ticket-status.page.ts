...
export class TicketStatusPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get<{ apiUrl: string   }>('assets/config.json').subscribe(cfg => {
      ...
      this.functionCode = "J3CRSod2zc0WWdXUyx8LEs-qz_eEUanGnu5Mv1BHRvhPAzFuMxTNBA==";
      if (ticketId && this.apiUrl) {
        this.http.get<TicketInfo>(`${this.apiUrl}/api/Ticket/GetTicketInfo/${ticketId}`,{ headers: this.buildHeaders() }).subscribe({
          next: info => {
            this.zone.run(() => {
          ...
          error: _ => {
            this.ticketInfo = {
              isSuccess: false,
              statusMessageEn: 'The ticket number you entered is not recognized. Please check your ticket or contact support.',
              statusMessageZh: '票號無效，請檢查票據或聯絡客服'
            };
            this.cd.markForCheck();
          }
        });
  ...
  onPay(method: 'visa' | 'mastercard' | 'cup' | 'jcb' | 'amex' | 'card') {
    ...
    this.http.post<PaymentInitResponse>(`${this.apiUrl}/api/Ticket/InitiatePayment`, body,{ headers: this.buildHeaders() })
      .subscribe({
  ...
  private buildHeaders(): HttpHeaders | undefined {
    if (this.functionCode ) {
      return new HttpHeaders({ 'x-functions-key': this.functionCode });
    }
    return undefined;
  }
